...
def convert_to_single_emb(x, offset: int = 32):
    ...
    if feature_num>32:
      print('feature_num exceeds max',feature_num,offset)
...
class GraphormerDataCollator:
    ...
    def __call__(self, features: List[dict]) -> Dict[str, Any]:
        ...
        batch["attn_bias"] = torch.zeros(batch_size, max_node_num + 1, max_node_num + 1, dtype=torch.float) # .to(device)
        batch["attn_edge_type"] = torch.zeros(batch_size, max_node_num, max_node_num, edge_feat_size, dtype=torch.long) # .to(device)
        batch["spatial_pos"] = torch.zeros(batch_size, max_node_num, max_node_num, dtype=torch.long) # .to(device)
        batch["in_degree"] = torch.zeros(batch_size, max_node_num, dtype=torch.long) # .to(device)
        batch["input_nodes"] = torch.zeros(batch_size, max_node_num, node_feat_size, dtype=torch.long) # .to(device)
        ...
        for ix, f in enumerate(features):
            for k in ["attn_bias", "attn_edge_type", "spatial_pos", "in_degree", "input_nodes", "input_edges"]:
                f[k] = torch.tensor(f[k]) # .to(device)
        ...
        if len(sample) == 1:  # one task
            if isinstance(sample[0], float):  # regression
                batch["labels"] = torch.from_numpy(np.concatenate([i["labels"] for i in features])) # .to(device)
            else:  # binary classification
                batch["labels"] = torch.from_numpy(np.concatenate([i["labels"] for i in features])) # .to(device)
        else:  # multi task classification, left to float to keep the NaNs
            batch["labels"] = torch.from_numpy(np.stack([i["labels"] for i in features], dim=0)) # .to(device)
